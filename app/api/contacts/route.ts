...
export async function GET() {
  ...
  try {
    ...
    const contacts: Array<{
      ...
      company?: string; source: string; detail: string; createdAt: Date;
    }> = [];
    ...
    for (const a of appointments) {
      ...
      contacts.push({ id: "appt_" + a.id, name: `${a.firstName} ${a.lastName}`, email: a.email, phone, source: "Booking", detail: a.serviceType.replace(/_/g, " ").toLowerCase().replace(/\b\w/g, (c: string) => c.toUpperCase()), createdAt: a.createdAt });
    }
